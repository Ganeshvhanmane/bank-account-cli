...
2. Withdraw money
3. Check balance
4. Show interest (Savings Account only — shows a message for Current Account)
5. Exit
...
- **Method Overriding (runtime polymorphism)** — `CurrentAccount` overrides `withdraw()` to allow overdraft. `SavingsAccount` and `CurrentAccount` both override `showAccountType()`. Confirmed with dynamic dispatch: a variable declared as `BankAccount` but holding a child object correctly calls the overridden version at runtime.
